...
<head>
  ...
  <meta name="description" content="Misfit Mafia YouTube Bot is a Discord membership verification application that verifies participating YouTube creator memberships and assigns the corresponding Discord roles.">
  <title>${title}</title>
  <style>
...
<body>
  <div class="wrap">
    <nav class="nav">
      <a class="brand" href="/"><span class="seal">MM</span><span><strong>Misfit Mafia YouTube Bot</strong></span></a>
...
app.get('/', (_req, res) => {
  res.status(200).send(shell('Misfit Mafia YouTube Bot', `
    <main>
      <section class="hero">
        <div class="eyebrow">Discord membership verification application</div>
        <h1>Misfit Mafia YouTube Bot</h1>
        <p><strong>Misfit Mafia YouTube Bot is a Discord membership verification application.</strong> It allows Discord users to verify whether the YouTube account connected to their Discord account has an active paid membership with a participating YouTube creator. If an active membership is found, Misfit Mafia YouTube Bot assigns or maintains the Discord role configured for that creator's YouTube membership level.</p>
        <a class="btn primary" href="/portal">Enter Verification Portal</a>
      </section>
      <section class="features">
        <article class="card"><div class="number">1</div><h2>Connect Discord</h2><p>When a user chooses to verify, Discord identifies the user's account and the YouTube identity connected to that Discord account. The public Misfit Mafia YouTube Bot homepage itself does not require login.</p></article>
        <article class="card"><div class="number">2</div><h2>Verify YouTube Membership</h2><p>Misfit Mafia YouTube Bot compares the connected YouTube identity with membership information from participating YouTube creator channels to determine whether an eligible paid membership is currently active.</p></article>
        <article class="card"><div class="number">3</div><h2>Assign the Correct Discord Role</h2><p>If an eligible membership is active, Misfit Mafia YouTube Bot uses the creator's configured YouTube membership level to assign or maintain the corresponding Discord role.</p></article>
      </section>
      <section class="card" style="margin-bottom:54px"><h2>Why Misfit Mafia YouTube Bot requests Google and YouTube access</h2><p>Participating YouTube creators authorize their own YouTube channels through Google OAuth. Misfit Mafia YouTube Bot requests access to read the authorized creator's YouTube channel information, channel membership levels, and current channel membership information. This information is used only to identify the authorized creator channel, verify active memberships, determine membership levels, and manage the corresponding Discord roles.</p><p>Google and YouTube data is not sold, used for advertising, or used to build advertising profiles.</p><div class="actions"><a class="btn secondary" href="/privacy">Read Privacy Policy</a><a class="btn secondary" href="/terms">Read Terms of Service</a></div></section>
    </main>`));
});

app.get('/portal', (_req, res) => {
  res.status(200).send(shell('Misfit Mafia YouTube Bot', `
    <section class="portal-title"><div class="eyebrow">Misfit Mafia YouTube Bot</div><h1>Verification Portal</h1><p>Choose what you need to do.</p></section>
